package com.fool.uk.hiddenwinner;
...
public class Createorder extends PageObjectModule {
	...
	public void HWordercreate() throws Throwable {
		...
		try {
			String name = read.readData("URL", 1, 2);
			//super.connect(name);
		     super.browser();
			String teur = read.readData("URL", 1, 1);
			super.getUrl(teur);
		...
		if (actUName.equals(expUName)) {
			Reporter.log("Motley Fool Hidden Winner Account Creation Test Passed. " + "Account created with Username: "
					+ str1);
			Thread.sleep(2000);
			driver.quit();
